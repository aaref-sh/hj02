...
<div transition:fade={{duration:150}} on:click|self={messagebox} class="fixed grid content-center z-10 top-0 left-0 right-0 bottom-0 bg-gray-800/80">
    <span on:click={messagebox} class="fixed top-0 right-0 text-sky-50 text-6xl select-none">×</span>   
    <div class="messagebox overflow-hidden m-auto w-4/5 sm:w-96 h-min std-border bg-stone-50/[98]  ">
        <h2 class="messegebox-header p-3 text-center text-blue-900 bg-graded">
        ...
        <div class="messagebox-body my-5 mx-8 min flex flex-col">
            <textarea class="std-textarea" rows="4" maxlength="150" />
...
<div class="post-container transition-all m-2 overflow-hidden std-border">
    <div class="post-header transition-all m-auto py-4 px-8 text-gray-800 bg-gradient-to-r from-white/20 to-cyan-200/[.15]">
        <h2 class="post-title">
            {post.title}
        </h2>
    ...
    <div class="post-body text-lg text-gray-700/[.95] py-4 px-6">
        ...
        <div class="images-grid grid grid-cols-2 justify-items-center sm:grid-cols-3 lg:gap-5 lg:grid-cols-4 gap-3 my-5">
            {#each images as image}
                <div on:click={()=>PreviewImage(image)} class="post-image cursor-zoom-in sm:brightness-50 hover:brightness-100 duration-300 transition-all bg-cover border border-red-900/40 hover:shadow-xl hover:scale-110 rounded-md h-40 w-full shadow-md" 
    ...
    <div class="contact my-2 mx-6 flex justify-start items-center">
        التواصل: 
        <button class="transition-all flex items-center px-1 mx-1 text-blue-500 hover:text-white rounded hover:bg-blue-500 border border-blue-400 ">
            <span class="material-symbols-outlined">
                smartphone
            </span>
            0951985386
        ...
        <a href="mailto:m.aaref-sh@hotmail.com" class="transition-all flex items-center px-1 mx-1 text-blue-500 hover:text-white rounded hover:bg-blue-500 border border-blue-400">
            <span class="material-symbols-outlined">
                mail
            </span>
    ...
    <div class="tags flex justify-between items-center my-2 mx-6  border-r-4 border-r-stone-600">
...
<style>
    ...
    .separator{
        @apply border-r border-sky-300/[.9];
    }
    .post-tag{
        @apply rounded-lg px-1 bg-stone-600 text-slate-100;
    }
</style>